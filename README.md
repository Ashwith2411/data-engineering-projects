# Data Engineering Portfolio

## About Me

Engineering graduate passionate about data engineering, ETL pipelines, and analytics. Skilled in Python, SQL, Pandas, PostgreSQL, and Power BI. Looking for entry-level Data Engineer or Data Analyst roles.

**📧 Email:** ashwith2411@gmail.com
**🔗 LinkedIn:**https://www.linkedin.com/in/ashwith24/

---

## Skills

| Category | Skills |
| :--- | :--- |
| Programming | Python (Requests, Pandas), SQL |
| Databases | PostgreSQL, MySQL |
| BI Tools | Power BI (DAX, Dashboards) |
| Data Engineering | ETL Pipelines, REST APIs, Data Modeling |
| Tools | Git, VS Code, pgAdmin |

---

## Projects

### Project 1: End-to-End ETL Pipeline

**Tech Stack:** Python, REST APIs, Pandas, PostgreSQL, Power BI

**What I Built:**
- ✅ ETL pipeline extracting data from REST API
- ✅ Data transformation and cleaning with Pandas
- ✅ Loaded to PostgreSQL database
- ✅ Interactive Power BI dashboard

**Features:**
- Automated data extraction
- Error handling and logging
- Scheduled daily runs
- Interactive dashboard with filters

**GitHub:**

---

## Connect With Me

- **LinkedIn:** https://www.linkedin.com/in/ashwith24/
- **GitHub:** https://github.com/Ashwith2411
- **Email:** ashwith2411@gmail.com

**I'm actively looking for entry-level Data Engineer or Data Analyst roles!**
